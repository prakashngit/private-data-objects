...
def CreateAndRegisterContract(config, enclaves, contract_creator_keys) :
    ...
    try:
        commit_id = initialize_response.commit_asynchronously(ledger_config, wait=30, use_ledger=use_ledger)
    except Exception as e:
    ...
    try:
        txn_id = ContractResponse.wait_for_commit(commit_id, use_ledger=use_ledger)
...
def UpdateTheContract(config, contract, enclaves, contract_invoker_keys) :
    ...
    for x in range(config['iterations']) :
        ...
        if ContractResponse.are_there_failed_past_commits():
            logger.error("Aborting rest of the execution: One or more previous commits failed")
            ErrorShutdown()
        ...
        if update_response.state_changed :
            ...
            try:
                commit_id = update_response.commit_asynchronously(ledger_config, wait=30, dependency_list_commit_ids=commit_dependencies, use_ledger=use_ledger)
    ...
    try:
        txn_id = ContractResponse.wait_for_commit(commit_dependencies[0], use_ledger=use_ledger)
        if use_ledger and txn_id is None:
